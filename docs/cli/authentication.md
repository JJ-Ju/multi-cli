...
1.  **Login with Google (Gemini Code Assist):**
    ...
    - Note that the web login must be done in a browser that can communicate with the machine Ionesco CLI is being run from. (Specifically, the browser will be redirected to a localhost url that Ionesco CLI will be listening on).
...
When running the Ionesco CLI in a non-interactive environment, you cannot use the interactive login flow.
...
For comprehensive guidance on using Ionesco CLI programmatically and in
