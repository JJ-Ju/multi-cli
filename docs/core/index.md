...
Ionesco CLI's core package (`packages/core`) is the backend portion of Ionesco CLI, handling communication with the Gemini API, managing tools, and processing requests sent from `packages/cli`. For a general overview of Ionesco CLI, see the [main documentation page](../index.md).
...
While the `packages/cli` portion of Ionesco CLI provides the user interface, `packages/core` is responsible for:
...
Ionesco CLI includes a model fallback mechanism to ensure that you can continue to use the CLI even if the default "pro" model is rate-limited.
